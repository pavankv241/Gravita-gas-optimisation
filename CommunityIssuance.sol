...
contract CommunityIssuance is ICommunityIssuance, OwnableUpgradeable, BaseMath {
	...
	function setAdminContract(address _admin) payable external onlyOwner {
	...
	function removeFundFromStabilityPool(uint256 _fundToRemove) payable external onlyOwner {
	...
	function issueGRVT() public override  onlyStabilityPool returns (uint256 issuance) {
		uint256 maxPoolSupply = GRVTSupplyCap;

		if (totalGRVTIssued >= maxPoolSupply) return 0;

		 issuance = _getLastUpdateTokenDistribution();
	...
	function _getLastUpdateTokenDistribution() internal view returns (uint256 totalDistribuedSinceBeginning) {
		...
		uint256 timePassed = (block.timestamp - lastUpdateTime) / SECONDS_IN_ONE_MINUTE;
		 totalDistribuedSinceBeginning = grvtDistribution * timePassed;
