...
contract BorrowerOperations is GravitaBase, IBorrowerOperations {
	...
	function openVessel(
		...
		ContractsCache memory contractsCache ;
			contractsCache.adminContract = adminContract;
			contractsCache.vesselManager = vesselManager;
			contractsCache.activePool = adminContract.activePool();
			contractsCache.debtToken = debtToken ;
	...
	function _adjustVessel(
		...
		require(_collWithdrawal <= vars.coll, "Trying to remove more than the vessel holds");
	...
	function _triggerBorrowingFee(
		...
		uint256 _debtTokenAmount
	) internal returns (uint256 debtTokenFee) {
		 debtTokenFee = _vesselManager.getBorrowingFee(_asset, _debtTokenAmount);
	...
	function _requireSingularCollChange(uint256 _collWithdrawal, uint256 _amountSent) internal pure {
		require(_collWithdrawal == 0 || _amountSent == 0, "Cannot withdraw & add coll");
	}
	...
	function _requireNonZeroAdjustment(
		...
		require(
			...
			"Must be either a collateral change or a debt change"
		);
	}
	...
	function _requireVesselIsActive(
		...
		require(status == 1, " Vessel does not exist or is closed");
	}
	...
	function _requireNonZeroDebtChange(uint256 _debtTokenChange) internal pure {
		require(_debtTokenChange !=  0, "Debt increase requires non-zero debtChange");
	}

	function _requireNotInRecoveryMode(address _asset, uint256 _price) internal view {
		require(!_checkRecoveryMode(_asset, _price), " not permitted during Recovery Mode");
	}

	function _requireNoCollWithdrawal(uint256 _collWithdrawal) internal pure {
		require(_collWithdrawal == 0, "not permitted Recovery Mode");
	}
	...
	function _requireICRisAboveCCR(address _asset, uint256 _newICR) internal view {
		require(_newICR >= adminContract.getCcr(_asset), "vessel must with ICR >= CCR");
	}

	function _requireNewICRisAboveOldICR(uint256 _newICR, uint256 _oldICR) internal pure {
		require(_newICR >= _oldICR, "Can't decrease your Vessel's ICR in Recovery Mode");
	}

	function _requireNewTCRisAboveCCR(address _asset, uint256 _newTCR) internal view {
		require(
			_newTCR >= adminContract.getCcr(_asset),
			" An operation with TCR < CCR is not permitted"
		);
	}
	...
	function _requireAtLeastMinNetDebt(address _asset, uint256 _netDebt) internal view {
		require(
			...
			"net debt must be greater than minimum"
		);
	}
	...
	function _requireValidDebtTokenRepayment(
		...
		require(
			...
			"Amount repaid must not be > the Vessel's debt"
		);
	}
	...
	function _requireSufficientDebtTokenBalance(
		...
		require(
			...
			"Caller doesnt have enough debt tokens to make repayment"
		);
	}
	...
	function _getNewNominalICRFromVesselChange(
		...
		uint256 _debtChange,
		bool _isDebtIncrease
	) internal pure returns (uint256 newNICR) {
		(uint256 newColl, uint256 newDebt) = _getNewVesselAmounts(
		 ...
		 newNICR = GravitaMath._computeNominalCR(newColl, newDebt);
		return newNICR;
	}
	...
	function _getNewICRFromVesselChange(
		...
		(uint256 newColl, uint256 newDebt) = _getNewVesselAmounts(
		 ...
		 newICR = GravitaMath._computeCR(newColl, newDebt, _price);
		return newICR;
	}
	...
	function _getNewTCRFromVesselChange(
		...
		newTCR = GravitaMath._computeCR(totalColl, totalDebt, _price);
		return newTCR;
	}
