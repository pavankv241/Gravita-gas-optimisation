...
contract DebtToken is IDebtToken, ERC20Permit, Ownable {
	...
	function _requireValidRecipient(address _recipient) internal view {
		require(
			...
			"Can't transfer directly to  token contract or the zero address"
		);
		require(_recipient != address(this),"error");
		require(
			address(stabilityPool) != _recipient,
			"Can't transfer  directly to the StabilityPool, VesselManager or BorrowerOps"
		);
		require(_recipient != vesselManagerAddress,"ERROR");
		require(_recipient != borrowerOperationsAddress,"error");
	}
	...
	function _requireCallerIsBOorVesselMorSP() internal view {
		require(
			...
			"Caller is neither BorrowerOperations nor VesselManager nor StabilityPool"
		);
	}
