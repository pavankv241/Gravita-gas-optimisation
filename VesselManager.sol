...
contract VesselManager is IVesselManager, GravitaBase {
	...
	function getNominalICR(address _asset, address _borrower) public view override returns (uint256 NICR) {
		(uint256 currentAsset, uint256 currentDebt) = _getCurrentVesselAmounts(_asset, _borrower);

		NICR = GravitaMath._computeNominalCR(currentAsset, currentDebt);
		return NICR;
	}
	...
	function getCurrentICR(
		...
		address _borrower,
		uint256 _price
	) public view override returns (uint256 ICR) {
		(uint256 currentAsset, uint256 currentDebt) = _getCurrentVesselAmounts(_asset, _borrower);
		 ICR = GravitaMath._computeCR(currentAsset, currentDebt, _price);
		return ICR;
	}

	// Get the borrower's pending accumulated asset reward, earned by their stake
	function getPendingAssetReward(address _asset, address _borrower) public view override returns (uint256 pendingAssetReward) {
	...
	) external override onlyVesselManagerOperations returns (uint256 newBaseRate ) {
	...
	function _updateStakeAndTotalStakes(address _asset, address _borrower) internal returns (uint256 newStake) {
		Vessel storage vessel = Vessels[_borrower][_asset];
		 newStake = _computeNewStake(_asset, vessel.coll);
	...
	function _calcRedemptionFee(uint256 _redemptionRate, uint256 _assetDraw) internal pure returns (uint256 redemptionFee) {
		 redemptionFee = _redemptionRate * _assetDraw / DECIMAL_PRECISION;
