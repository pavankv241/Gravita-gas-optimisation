...
contract FeeCollector is IFeeCollector, OwnableUpgradeable {
	...
	function simulateRefund(
		...
		_paybackFraction == 1e18 ? mRecord.amount - expiredAmount: (((mRecord.amount - expiredAmount) * _paybackFraction) / 1 ether);
	...
	function _decreaseDebt(
		...
		uint256 NOW = block.timestamp;
		require(_paybackFraction <= 1 ether, "Payback fraction cannot be > 1 (@ 10**18)");
		require(_paybackFraction != 0, "Payback fraction cannot be zero");
		FeeRecord storage sRecord = feeRecords[_borrower][_asset];
		if (sRecord.amount == 0) {
			return;
		}
		if (sRecord.to <= NOW) { // @audit
	...
	function _updateFeeRecord(
		...
		if (NOW < _sRecord.from) {
		 ...
		 expiredAmount = _calcExpiredAmount(_sRecord.from, _sRecord.to, _sRecord.amount);
	...
	function _calcExpiredAmount(
		...
		uint256 _to,
		uint256 _amount
	) internal view returns (uint256 expiredAmount) {
		uint256 NOW = block.timestamp;
		if (_from > NOW) {
			return 0;
		}
		...
		uint256 decayRate = (_amount * PRECISION) / lifeTime;
		 expiredAmount = (elapsedTime * decayRate) / PRECISION;
		return expiredAmount;
	}
