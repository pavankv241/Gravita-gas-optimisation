...
contract LockedGRVT is Ownable, Initializable {
	...
	function addEntityVesting(address _entity, uint256 _totalSupply) public payable onlyOwner {
	...
	function lowerEntityVesting(address _entity, uint256 newTotalSupply) public payable onlyOwner entityRuleExists(_entity) {
		sendGRVTTokenToEntity(_entity);
		Rule storage vestingRule = entitiesVesting[_entity];

		require(newTotalSupply > vestingRule.claimed, "Total Supply goes <= the claimed total.");

		vestingRule.totalSupply = newTotalSupply;
	}

	function removeEntityVesting(address _entity)  public payable onlyOwner entityRuleExists(_entity) {
		sendGRVTTokenToEntity(_entity);
		Rule storage vestingRule = entitiesVesting[_entity];
	...
	function sendGRVTTokenToEntity(address _entity) private {
		...
		entityRule.claimed = entityRule.claimed + unclaimedAmount;

		assignedGRVTTokens = assignedGRVTTokens - unclaimedAmount;
		grvtToken.safeTransfer(_entity, unclaimedAmount);
	}

	function transferUnassignedGRVT() external payable onlyOwner {
