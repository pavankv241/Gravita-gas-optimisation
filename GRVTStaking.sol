...
contract GRVTStaking is IGRVTStaking, PausableUpgradeable, OwnableUpgradeable, BaseMath, ReentrancyGuardUpgradeable {
	...
	function changeTreasuryAddress(address _treasury) public payable onlyOwner {
		treasury = _treasury;
	...
	function _getPendingAssetGain(address _asset, address _user) internal view returns (uint256 AssetGain) {
		uint256 F_ASSET_Snapshot = snapshots[_user].F_ASSETS_Snapshot[_asset];
		 AssetGain = stakes[_user] * (F_ASSETS[_asset] - F_ASSET_Snapshot) / DECIMAL_PRECISION;
		return AssetGain;
	}
