...
contract StabilityPool is ReentrancyGuardUpgradeable, GravitaBase, IStabilityPool {
	...
	function _computeGRVTPerUnitStaked(uint256 _GRVTIssuance, uint256 _totalDeposits) internal returns (uint256 GRVTPerUnitStaked) {
		/*
		...
		uint256 GRVTNumerator = (_GRVTIssuance * DECIMAL_PRECISION) + lastGRVTError;
		 GRVTPerUnitStaked = GRVTNumerator / _totalDeposits;
	...
	function _computeRewardsPerUnitStaked(
		...
		require(_debtToOffset <= _totalDeposits, "Debt is larger than totalDeposits");
	 ...
	 * @return assetGain the gain
	 */
	function _getGainFromSnapshots(
		uint256 initialDeposit,
		Snapshots storage snapshots,
		address asset
	) internal view returns (uint256 assetGain) {
		/*
		...
		assetGain = (initialDeposit * (firstPortion + secondPortion)) / P_Snapshot / DECIMAL_PRECISION;
		...
		returns (uint256 GRVTGain)
	{
		...
		GRVTGain = (initialStake * (firstPortion + secondPortion)) / P_Snapshot / DECIMAL_PRECISION;
	...
	function _getCompoundedStakeFromSnapshots(uint256 initialStake, Snapshots storage snapshots)
		internal
		view
		returns (uint256 compoundedStake)
	{
	...
	function _requireNoUnderCollateralizedVessels() internal {
		...
		for (uint256 i = 0; i < assetsLen; ) {
			...
			require(
				...
				"Cannot withdraw while there are vessels with ICR < MCR"
			);
			unchecked {
				++i;
			}
	...
	function _requireUserHasDeposit(uint256 _initialDeposit) internal pure {
		require(_initialDeposit != 0, "User must have a non-zero deposit");
	}
